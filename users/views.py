...
class UserRegistrationView(generic.CreateView):
	"""
	The registration view for the user. This is a generic view that will handle the creation of a new user.
	@param form_class - the form class for the registration form.
	@param template_name - the template for the registration form.
	@param success_url - the url to redirect to after a successful registration.
	"""
...
class PasswordUpdateView(PasswordChangeView):
	"""
	When the form is valid, we want to redirect the user to a success page. We can do this by calling the super().form_valid(form) method.
...
class EditUserDetailsForm(LoginRequiredMixin, generic.UpdateView):
	"""
	When the form is valid, we want to redirect the user to the success page. We also want to           
	display a success message to the user. We override the form_valid method to do this.           
	@param form - the form we are validating           
...
class EditUserProfile(LoginRequiredMixin, generic.UpdateView): 
	"""
	When the form is valid, we want to redirect the user to the profile page. We also want to display a success message.
